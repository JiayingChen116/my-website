...
```{r fetch_db_data, fig.width=11, warning = FALSE}
#connect to the downloaded database
cdc_database <- DBI::dbConnect(
  drv = RSQLite::SQLite(),
  dbname = "cdc_data.db" 
)
```

```{r lookAtDB, fig.width=11, warning = FALSE}
#show existing tables in the connected db
DBI::dbListTables(cdc_database)
```

```{r setUpTable, fig.width=11, warning = FALSE}
#assign the cdc table to the cdc_data dataframe
cdc_data <- dplyr::tbl(cdc_database, "cdc")
cdc_data

#we use simply tidyverse language to have our queries translated into SQL code. We do this for two seperate queries: one for each plot (one including medcond_yn, and the other including icu_yn)
query1 <-   cdc_data %>%
  select("sex", "age_group", "medcond_yn", "death_yn") %>%
  group_by(sex, age_group, medcond_yn, death_yn) %>% 
  summarise(n = n()) %>%
  mutate(deathRate = n / sum(n))
...
#we assign the results from our queries to tibbles/data frames
q1Tibble <- query1 %>% 
  collect()
...
```{r adjust_tibble, fig.width=11, warning = FALSE}

#we filter out data which we do not need for the reproduction of the graph.
q1TibbleAdj <- q1Tibble %>%
  filter(!sex %in% c("Other", "Missing", "Unknown", "NA"), !age_group %in% c("Missing", "Unknown", "NA"), !medcond_yn %in% c("Missing", "Unknown", "NA"), !death_yn %in% c("Missing", "Unknown", "NA"))
```

```{r splitUpDeathRates, fig.width=11, warning = FALSE}
#we calculate death rates after grouping by the necessary variables
splitDeathRates <- q1TibbleAdj %>% 
  group_by(sex, age_group, medcond_yn, death_yn) %>% 
  summarise(n = sum(n)) %>% 
  mutate(deathRate = n/sum(n))

#let's view what we have created. We can see that the deathRate is split up by sex, age group, and the presence co-morbidities which we want, in order to be able to reproduce the graph
head(splitDeathRates)
```

```{r facet_splitDeathRates, fig.height=8, fig.width=13}

facet_DeathRates_medcond <- splitDeathRates %>% 
  filter(death_yn == "Yes") %>%
   mutate(medcond_yn = recode(medcond_yn, #We recode the values for the co-morbidities coloumn, in order to be able to view the wanted plot labels
    "Yes" = "With Comorbidities",
    "No" = "Without Comorbidities",
  )) %>% 
  ggplot()+
    geom_col(aes(y = age_group, x = deathRate), fill = "#687ca8")+
  
    geom_text(aes(x = deathRate, y = age_group, label = percent(round(deathRate, 3))), color = "black", size = 5, hjust = -0.1, vjust = 0.3)+ #we add data labels

    facet_grid(fct_relevel(medcond_yn, "With Comorbidities", "Without Comorbidities") ~ sex)+ #for the first time, we use facet_grid in order to be able to specify our plot rows and columns 
...
facet_DeathRates_medcond
```

```{r adjust_tibble2, fig.width=11, warning = FALSE}

#we filter out data which we do not need for the reproduction of the graph.
q2TibbleAdj <- q2Tibble %>%
  filter(!sex %in% c("Other", "Missing", "Unknown", "NA"), !age_group %in% c("Missing", "Unknown", "NA"), !icu_yn %in% c("Missing", "Unknown", "NA"), !death_yn %in% c("Missing", "Unknown", "NA"))
```

```{r splitUpDeathRates2, fig.width=11, warning = FALSE}
#we calculate death rates after grouping by the necessary variables
splitDeathRates2 <- q2TibbleAdj %>% 
  group_by(sex, age_group, icu_yn, death_yn) %>% 
  summarise(n = sum(n)) %>% 
  mutate(deathRate = n/sum(n))

#let's view what we have created. We can see that the deathRate is split up by sex, age group, and the presence co-morbidities which we want, in order to be able to reproduce the graph
head(splitDeathRates2)
```

```{r facet_splitDeathRates2, fig.height=8, fig.width=13}

facet_DeathRates_icu <- splitDeathRates2 %>% 
  filter(death_yn == "Yes") %>%
   mutate(icu_yn = recode(icu_yn,#We recode the values for the ICU Admission coloumn, in order to be able to view the wanted plot labels
    "Yes" = "ICU Admission",
    "No" = "No ICU Admission",
  )) %>% 
  ggplot()+
    geom_col(aes(y = age_group, x = deathRate), fill = "#ff8f7c")+
  
    geom_text(aes(x = deathRate, y = age_group, label = percent(round(deathRate, 2))), color = "black", size = 5, hjust = -0.1, vjust = 0.3)+ #we add data labels

    facet_grid(fct_relevel(icu_yn, "ICU Admission", "No ICU Admission") ~ sex)+ #for the second time now, we use facet_grid in order to be able to specify our plot rows and columns 
